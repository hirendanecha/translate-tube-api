...
var Appointments = function (appointment) {
  ...
  this.translatorProfileId = appointment.translatorProfileId;
  this.translatorName = appointment.translatorName;
...
Appointments.findAndSearchAll = async (
  ...
  let whereCondition = `${search ? `a.translatorName LIKE '%${search}%'` : ""}`;
  ...
  const searchData = await executeQuery(
    `SELECT a.id, a.translatorName, a.appointmentDateTime, a.createdDate,a.profileId,a.isCancelled,p.FirstName,p.LastName,p.ProfilePicName,p.Username FROM appointments as a left join profile as p on p.ID = a.profileId  ${
      whereCondition ? `WHERE ${whereCondition} ` : ""
...
Appointments.getPractitionerAppointments = async (id) => {
  const query =
    "select a.*,p.Username,p.FirstName,p.LastName,p.ProfilePicName from appointments as a left join profile as p on p.ID = a.profileId where a.isCancelled = 'N' and a.translatorProfileId = ?";
  const values = [id];
...
Appointments.getUserAppointments = async (id) => {
  const query =
    "select a.*,p.Username,p.FirstName,p.LastName,p.ProfilePicName from appointments as a left join profile as p on p.ID = a.translatorProfileId where a.isCancelled = 'N' and a.profileId = ?";
  const values = [id];
  const appointmentList = await executeQuery(query, values);
  if (appointmentList) {
    return appointmentList;
