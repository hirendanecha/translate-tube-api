...
exports.createAppointment = async (req, res) => {
  ...
  const appointmentData = {
    ...
    translatorProfileId: data.translatorProfileId,
  ...
  if (appointmentData) {
    ...
    if (oldAppointment.length) {
      return res.send({
        error: false,
        message:
          "Appointments already book with other Client, please select other slots!",
      });
    } else {
      const id = await Appointments.createAppointments(appointmentData);
      const emailData = {
        profileId: appointmentData.profileId,
        translatorProfileId: appointmentData.translatorProfileId,
        topics: topics,
        slug: slug,
