...
exports.notificationMail = async (userData) => {
  ...
  const mailObj = {
    email: userData.email,
    subject: "Translate notification",
...
exports.channelNotificationEmail = async (userData) => {
  ...
  let msg = `You have been assign in TranslateTube channel by the TranslateTube Admin.
  To access your channel, log into your TranslateTube account,click on the
  TranslateTube icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `TranslateTube has approved your Translator application.`;
    } else {
      msg = `TranslateTube has unapproved your Translator application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
...
exports.cancelAppointmentNotificationMail = async (id, translatorName) => {
  const query =
    "select u.Email,p.FirstName,p.LastName,p.Username from users as u left join profile as p on p.UserID = u.Id where p.ID =?";
  const values = [id];
  const [data] = await this.executeQuery(query, values);
  let name = data?.Username || userData.FirstName;
  let msg = `Your appointment with ${translatorName} has been cancelled, please book another slot!`;
  let redirectUrl = `${environment.FRONTEND_URL}`;
...
exports.sendAppointmentMailToUser = async (data) => {
  ...
  const query1 =
    "select u.Email,p.FirstName,p.LastName,p.Username from users as u left join profile as p on p.UserID = u.Id where p.ID =?";
  const values1 = [data.translatorProfileId];
...
const getIcalObjectInstance = async (
  ...
  //   cal.domain("translate.tube");
  cal.createEvent({
    start: starttime, // eg : moment()
    end: moment(starttime).add(30, "min"),
    description: description, // 'More description'
    url: url, // 'event url',
    summary: "Request video call with lawyers",
    organizer: {
      // 'organizer details'
      name: name,
      email: email,
